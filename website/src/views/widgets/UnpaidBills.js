...
const UnpaidBills = (data) => {
  let bills = data.data.data.unpaid;
  console.log(bills);
  ...
  return (
    <>
      ...
      >
        ...
        <CModalFooter>
          <CRow className={"btn-block"}>
            <CCol xs="6">
              <CButton color="secondary" disabled className={"btn-block"} onClick={() =>     (function() {pay(bills[selectedBill].id); setModalActive(!showModal)})()    }>Insufficient wallet balance</CButton>{' '}
            </CCol>
            <CCol xs="6">
              <CButton color="primary" className={"btn-block"}onClick={() =>     (function() {pay(bills[selectedBill].id); setModalActive(!showModal)})()    }>Pay with credit card</CButton>{' '}
            </CCol>
          </CRow>
        </CModalFooter>
      </CModal>
